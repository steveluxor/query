# Query - Agent Runtime 架构 (v5: DAG 校验分层 + WorkflowValidator)
...
Orchestrator (生命周期管理)
     |
     | 1. WorkflowValidator → DAG 结构校验 (cycle/dependency)
     | 2. Registry.validate_capabilities → Agent 能力校验
     | 3. 查 Registry → 获取 Agent Capability
     | 4. 校验 inputs 是否满足 (has_all_outputs)
     | 5. execute(Agent)
     | 6. retry: clear_outputs(output_keys) + merge(merge_policy)
```
...
| 组件 | 负责 | 校验项 |
|------|------|--------|
| `WorkflowValidator` | DAG 结构是否合法（与 Agent 无关） | 空图检查、依赖存在性、循环检测、分层计算 |
| `AgentRegistry` | Agent 组合是否合法（基于 Capability 契约） | Agent 注册检查、inputs 前置校验、输出冲突 |
| `Orchestrator` | 协调调用两者，不关心内部规则 | — |
...
v5 (Runtime 成熟):
  Registry.validate_dag() 混合结构+能力    WorkflowValidator + Registry 分层校验
  DAG 结构 & Agent 能力耦合               get_layers() → validate_capabilities(plan, layers)
  Orchestrator 调单一校验                  Orchestrator 调双层校验
  无 Capability inputs 校验               layer-based inputs 前置校验
```
...
└── app/
    ...
    │   ├── agent_registry.py    # Agent 能力注册表 (Capability 校验 + 实例绑定)
    │   ├── workflow_validator.py # DAG 结构校验器 (v5: 新增)
...
class AgentRegistry:
    ...
    def validate_capabilities(plan, layers)       # Agent 能力校验 (v5: 接受 layers 参数)
    def format_for_prompt()                       # 生成 planner prompt 片段
```

### Agent 能力校验 (v5: 从 validate_dag 拆分)
...
| requires/inputs 校验 | 该 Agent 的 inputs 是否在上游产出 | `analysis 缺少输入 'evidence'（无上游产出）` |
| 输出冲突检测 | 同层多个 task 写同一 output_key | `输出冲突: [t1, t2] 都写入 'evidence'` |

requires/inputs 校验采用 **layer-based** 模型：同层所有 Agent 输出累计为上游产出，下一层 Agent 基于该集合检查 inputs 是否满足。
...
## WorkflowValidator — DAG 结构校验器 (v5: 新增)

```python
class WorkflowValidator:
    def validate_structure(plan: TaskGraph) -> list[str]    # 结构校验
    def get_layers(plan: TaskGraph) -> dict[str, int]       # 分层计算
```

职责：校验 TaskGraph 本身是否合法，与 Agent 能力无关。

| 校验项 | 说明 | 错误示例 |
|--------|------|----------|
| 空图检查 | Planner 生成空计划应视为非法 | `TaskGraph 不能为空` |
| 依赖存在性 | depends_on 引用的 task 必须存在 | `依赖不存在: t2 -> t3` |
| 循环检测 | 拓扑排序，无环才合法 | `检测到循环依赖` |
| 分层计算 | 按 depends_on 计算每个 task 的深度 | 返回 `{"t1": 0, "t2": 1}` |

### 演进方向

`WorkflowValidator` 未来可扩展为 `WorkflowAnalyzer`，提供更多 DAG 分析能力：
- `topological_sort()` — 拓扑排序执行顺序
- `find_parallel_tasks()` — 可并行执行的任务分组
- `get_execution_order()` — 完整执行顺序
...
**规划模式**: Planner 拆多步 → WorkflowValidator 校验 → Registry 能力校验 → 逐步执行 → Generate → Critic
...
Planner 生成 TaskGraph
         |
WorkflowValidator.validate_structure()
         | 结构合法
WorkflowValidator.get_layers()
         |
AgentRegistry.validate_capabilities(plan, layers)
         | 能力合法
...
- `agent_orchestrator.py` — Agent 编排器 (v5: WorkflowValidator 双层校验)
  ...
  - MCP session 生命周期管理
  - 规划模式: WorkflowValidator 结构校验 → Registry 能力校验 → DAG 执行

- `workflow_validator.py` — DAG 结构校验器 (v5: 新增)
  - `validate_structure()` — 空图 + 依赖存在性 + 循环检测
  - `get_layers()` — 按 depends_on 深度分层

- `agent_registry.py` — Agent 能力注册表 (v5: validate_capabilities)
  - `validate_capabilities(plan, layers)` — Agent 注册检查 + inputs 校验 + 输出冲突检测
...
python -m pytest tests/test_workflow_validator.py -v     # v5: 新增
...
| DAG 结构校验 | `core/workflow_validator.py` | validate_structure() + get_layers() (v5: 新增) |
| Agent 能力校验 | `core/agent_registry.py` | validate_capabilities(plan, layers) (v5: 从 validate_dag 拆分) |
