...
def create_mcp_tools(mcp_client: MCPClient, session_id: str = "", include: list[str] | None = None):
    ...
    @tool
    async def search_documents(query: str, row_start: int = None, row_end: int = None,
    ...
    async def calculate_sum(key_name: str, row_filter: str = "", content_filter: str = "") -> str:
        """对已检索到的文档内容中指定列（key）的数值进行精确求和。当用户问"总共"、"合计"、"一共多少钱"等加总问题时调用。数据已由上游检索就绪，直接调用即可。
    ...
    async def calculate_rank(key_name: str, ascending: bool, position: int = 1, content_filter: str = "") -> str:
        """从已检索到的文档内容中，对指定列（key）的数值排序并返回第N名的记录。当用户问"最贵"、"最便宜"、"第三高"等排名问题时调用。ascending=true=升序(最便宜/最低)，false=降序(最贵/最高)。数据已由上游检索就绪，直接调用即可。
    ...
    async def read_all_rows() -> str:
        """读取当前搜索到的文档的全部数据行。当需要完整信息（如列出所有品牌、所有记录、完整清单）时调用。当前 search_documents 只返回部分数据，调用此工具可获取全文。数据已由上游检索就绪，直接调用即可。"""
